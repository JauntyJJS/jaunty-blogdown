...
[Docker](https://www.docker.com/) 🐳 [@DOCKER] has been gaining in popularity in the academic field to ensure scripts that are used to run on research data can be easily opened and explored by others. A picture friendly introduction of Docker can be found in this [Devopedia webpage](https://devopedia.org/docker).
...
In this blog, I will try to share what I have learnt by providing a step by step walk-through with pictures to show on how to install Docker 🐳 in Windows 10 and use it recreate two lipidomics software web service 🌐, one made using [Django](https://www.djangoproject.com/) called [LINEX](https://exbio.wzw.tum.de/linex/) [@LINEX2021], [@LINEX2022] and the the other using [Shiny](https://shiny.rstudio.com/) called [MoSBi](https://exbio.wzw.tum.de/mosbi/) [@MoSBi]. Both software are from [LpiTUM](https://www.lipitum.de/Home). I pick these two software because the end results is tangible and do not take too long to build using Docker (once it is successfully installed).
...
![LINEX_in_D_drive](images/LINEX_in_D_drive.png)
...
![MoSBi_in_D_drive](images/MoSBi_in_D_drive.png)
