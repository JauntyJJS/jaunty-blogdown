...
From our previous failed attempt, tt is also hard to convince ourselves and others that 
...
I then use the function from the `pointblank` package to detect if there are any issues.
...
We use `tidyxl` functions to read the excel file in cells.
...
We use `tidyxl` functions to read the excel file in formats.
...
We identify the `local format id` accordingly.
...
We continue with `sample_excel_attempt_3` which reads the `text_integer_issue` column as text instead. We use some functions from the `pointblank` package to ensure that the text are positive integers.
...
Similarly, we can use `pointblank` to ensure that the text are positive numbers. Do note that when numeric are read as text in this dataset. It can be of the form 
...
We use `pointblank::col_vals_expr` to do a validation using the `is_integer_vector` function. 
